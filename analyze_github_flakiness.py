...
class HistoricalFlakinessAnalyzer:
    ...
    def analyze_pr(self, pr_info):
        ...
        # (GitHub/Konflux API doesn't expose actual pipeline reruns)
        total_retests = self.count_retest_comments(pr_number)
...
def main():
    ...
    # Analyze last 7 days to match dashboard timeframe
    days_back = 7
    ...
    print(f"\n📊 Total: {overall_prs} PRs, {overall_retests} /retest comments ({overall_prs_with_retests/overall_prs*100:.1f}% rate)" if overall_prs > 0 else "\n📊 No PRs analyzed")
